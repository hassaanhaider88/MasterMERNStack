...
# Life Cycle Events / Special methods / Hooks

- Special methods hooks provided by nestjs.
- automatically called different stages of a module/service/component life
- Used to perform actions during creation or destruction.

- onModulehit() => Called when the modole is initialized.
- onModuleDestroy() => Called before the module is distroyed.
- afterModuleInit() (Optional) => called after all modules are intialized
- onApplicationBootstrap() => when app is fully bootstrapped.
- onApplicationShutdown() => Called when app is shutting down.

# Enviremonet Variables

Enviroment variables are used to stroe values that change depending on the
enviroment ( developement, product, testing) like databseUrls, APIkeys secrets. etc.
use @nestjs/config package and then in app.module.ts file in import array add this line
```js
